          ...
          <table role="presentation" width="100%" style="max-width:520px;background:#ffffff;border-radius:12px;border:1px solid #e4e4e7;">
            <tr>
              <td style="padding:24px;color:#18181b;font-size:15px;line-height:1.6;">
                ...
                ${notesBlock}
                <p style="margin:20px 0 0;font-size:12px;color:#71717a;">BAMYS Marketplace automated notification</p>
              </td>
            </tr>
          </table>
        </td>
    ...
    if (apiKey) {
      ...
      try {
        const { error } = await resend.emails.send({
          ...
          to,
          subject: "Cart approval request — BAMYS Marketplace",
  ...
  if (apiKey && baseUrl) {
    ...
    try {
      const { error } = await resend.emails.send({
        from: "noreply@qevira.michaelalene.com",
        to,
        subject: "Your cart has been approved - BAMYS",

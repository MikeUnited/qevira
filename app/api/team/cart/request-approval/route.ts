      ...
      <tr>
        <td align="center">
          <table role="presentation" width="100%" style="max-width:520px;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e4e4e7;">
            ...
            <tr>
              <td style="padding:16px 24px;background:#fafafa;border-top:1px solid #e4e4e7;color:#71717a;font-size:12px;">BAMYS Marketplace automated notification</td>
            </tr>
          </table>
        </td>
      </tr>
    ...
    for (const d of directors) {
      ...
      try {
        const { error } = await resend.emails.send({
          ...
          to,
          subject: "Cart Approval Required - BAMYS Marketplace",
          html,
        });
        if (error) {
          console.error(

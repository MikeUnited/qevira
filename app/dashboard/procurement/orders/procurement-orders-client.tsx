...
function isOrderCancelled(order: ProcurementOrder): boolean {
  if (procurementDocstatus(order) === 2) return true;
  return order.status.trim().toLowerCase() === "cancelled";
}

/** Mirrors confirm-receipt API: submitted SO, not cancelled, not already confirmed. */
function canConfirmReceipt(
  order: ProcurementOrder,
  confirmedOrderIds: ReadonlySet<string>
): boolean {
  if (confirmedOrderIds.has(order.orderId)) return false;
  if (isOrderCancelled(order)) return false;
  return procurementDocstatus(order) === 1;
}

function canActOnReceipt(teamRole: string | undefined): boolean {
  return teamRole !== "PHARMACIST";
}
...
export function ProcurementOrdersClient() {
  ...
  const [receiptDialogOrderId, setReceiptDialogOrderId] = React.useState<
    string | null
  >(null);
  const [receiptSubmitting, setReceiptSubmitting] = React.useState(false);
  const [confirmedOrderIds, setConfirmedOrderIds] = React.useState<
    Set<string>
  >(() => new Set());
  const [teamRole, setTeamRole] = React.useState<string | undefined>(undefined);
  ...
  const fetchReceiptConfirmations = React.useCallback(async () => {
    try {
      const res = await fetch("/api/dashboard/procurement/orders/receipts", {
        credentials: "include",
      });
      ...
      if (!res.ok) return;
      const ids = Array.isArray(json.orderIds)
        ? json.orderIds.filter((id): id is string => typeof id === "string")
        : [];
      setConfirmedOrderIds(new Set(ids));
    } catch {
      /* non-fatal */
    }
  }, []);

  const fetchTeamRole = React.useCallback(async () => {
    try {
      ...
      if (!res.ok) return;
      setTeamRole(
        typeof json.teamRole === "string" ? json.teamRole : undefined
      );
    } catch {
      /* non-fatal */
    }
  ...
  React.useEffect(() => {
    void fetchOrders();
    void fetchReceiptConfirmations();
    void fetchTeamRole();
  }, [fetchOrders, fetchReceiptConfirmations, fetchTeamRole]);
  ...
  async function submitReceiptConfirmation() {
    if (!receiptDialogOrderId) return;
    setReceiptSubmitting(true);
    try {
      const res = await fetch(
        "/api/dashboard/procurement/orders/confirm-receipt",
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ orderId: receiptDialogOrderId }),
        }
      );
      const json = (await res.json().catch(() => ({}))) as {
        error?: string;
      ...
      toast.success("Receipt confirmed. Your inventory has been updated.");
      setConfirmedOrderIds((prev) => {
        const next = new Set(prev);
        next.add(receiptDialogOrderId);
        return next;
      });
      setReceiptDialogOrderId(null);
    } catch {
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
      ...
      <Dialog
        open={receiptDialogOrderId !== null}
        onOpenChange={(open) => {
          if (!open && !receiptSubmitting) setReceiptDialogOrderId(null);
        }}
      >
        <DialogContent showCloseButton>
          <DialogHeader>
            ...
            <DialogDescription>
              Confirm that you have received all items in order{" "}
              <span className="font-mono font-medium text-foreground">
                {receiptDialogOrderId}
              </span>
              . Stock will be recorded in your buyer warehouse.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="border-t-0 bg-transparent p-0 pt-2 sm:justify-end">
            <Button
              type="button"
              variant="outline"
              onClick={() => setReceiptDialogOrderId(null)}
            ...
            <Button
              type="button"
              onClick={() => void submitReceiptConfirmation()}
              disabled={receiptSubmitting}
            >
              {receiptSubmitting ? (
                <>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-4">
          {displayedOrders.map((order) => {
            ...
            const receiptConfirmed = confirmedOrderIds.has(order.orderId);
            const receiptEligible = canConfirmReceipt(
              order,
              confirmedOrderIds
            );
            const receiptActor = canActOnReceipt(teamRole);
            return (
              <Card
                key={order.orderId}
                className={cn(
                    ...
                    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                      <div className="flex flex-wrap items-center gap-2">
                        {receiptConfirmed ? (
                          <Badge
                            variant="secondary"
                            className="gap-1.5 border-emerald-200 bg-emerald-50 text-emerald-900 dark:border-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-100"
                          >
                            <CheckCircle2 className="size-3.5" aria-hidden />
                            Receipt confirmed
                          </Badge>
                        ) : receiptEligible && receiptActor ? (
                          <Button
                            type="button"
                            className="gap-2"
                            onClick={(e) => {
                              e.stopPropagation();
                              setReceiptDialogOrderId(order.orderId);
                            }}
                          >
                          ...
                          <p className="text-muted-foreground text-sm">
                            Only Directors and Owners can confirm receipt.
                          </p>
                        ) : null}
                      </div>
                      {showCancelRequest ? (
